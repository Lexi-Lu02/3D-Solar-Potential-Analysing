...
| `context` | object \| null | 见 §7.10 | 前端预先计算/拉取好的可信数据（当前选中的建筑 / precinct / 用户调过的假设）。后端注入到 system prompt 让 LLM 直接使用而**不浪费工具调用去验证** |
...
### 7.10 前端可信上下文 `context` 字段

前端有些数据是自己算出来的（年节省 / 回本年限 / 自定义电价），或是从其他端点
预先拉好的（地址、solar score、annual kWh）。如果只把它们塞在 user 消息里，LLM
会被 system prompt 的"必须用工具验证"规则绊住，**白浪费一次工具调用**去查同样
的数据。

`context` 字段就是为此设计的**可信通道**：
...
POST /api/v1/ai/chat
{
  "user_type": "property_owner",
  "messages": [
    { "role": "user", "content": "Is this building suitable for solar?" }
  ],
  "context": {
    "selected_building": {
      "id": 12208,
      "structure_id": 806089,
      "address": "1 Marne Street, South Yarra, ...",
      "solar_score": 3.8,
      "annual_kwh": 100523,
      "usable_roof_area_m2": 247.4,
      "install_cost_aud": 37110,
      "annual_savings_aud": 25131,
      "payback_years": 1.5,
      "co2_offset_kg_yr": 79000
    },
    "assumptions": {
      "electricity_tariff_aud_per_kwh": 0.30,
      "self_consumption_pct": 100,
...
后端会把 `context` 渲染成 Markdown 区块**附加到 system prompt 末尾**：

```
# Pre-computed context (trusted)
The frontend has already prepared the following data for the user, drawn
from the same database / formulas your tools would use. You MAY cite these
numbers directly without re-verifying them via tools. ...

## Selected building
- id: 12,208
- annual_kwh: 100,523
- install_cost_aud: 37,110
...
```

LLM 看到的是 system 消息里的"可信数据"，不是 user 消息里的"用户输入"，所以
不会重新调工具验证。**典型效果：原本 37s 的请求降到 ~10s。**

#### 可用字段（[`app/services/ai_context.py`](app/services/ai_context.py)）

`selected_building` (BuildingContext)：
| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | int (必填，≥1) | `buildings.id` |
| `structure_id` | int \| null | City of Melbourne 原始 id |
| `address` | string \| null (≤300) | |
| `solar_score` | float \| null (0-5) | |
| `dominant_rating` | string \| null (≤40) | |
| `usable_roof_area_m2` | float \| null (≥0) | |
| `total_roof_area_m2` | float \| null (≥0) | |
| `max_panels` | int \| null (≥0) | |
| `system_size_kw` | float \| null (≥0) | |
| `annual_kwh` | float \| null (≥0) | |
| `install_cost_aud` | float \| null (≥0) | |
| `annual_savings_aud` | float \| null (≥0) | |
| `payback_years` | float \| null (≥0) | |
| `lifetime_savings_aud` | float \| null (≥0) | |
| `co2_offset_kg_yr` | float \| null (≥0) | |
| `equivalent_trees` | float \| null (≥0) | |

`selected_precinct` (PrecinctContext)：`precinct_id` (必填) + name / postcode / total_kwh_annual / total_usable_area_m2 / installed_capacity_kw / potential_capacity_kw / adoption_gap_kw / building_count / rank。

`assumptions` (Assumptions)：`electricity_tariff_aud_per_kwh` (0-10) / `self_consumption_pct` (0-100) / `panel_efficiency` (0-1) / `performance_ratio` (0-1) / `season` (`annual`/`summer`/`autumn`/`winter`/`spring`)。

**全部字段都是 optional**，前端只填它当前知道的部分；null/未填的字段会从注入的
区块里直接跳过，不会显示。

#### 安全特性

- 整个 schema 是**严格 typed** —— 没有任意 string / dict / json blob，所以**用户
  无法通过 `context` 做 prompt injection**
- 字符串字段都有长度上限
- 数值字段都有范围下/上界，传越界值后端直接 422
- 真正的用户输入仍然走 `messages[]`，仍然被 `<user_query>` 包裹（不可信）

### 7.11 Persona 切换（property owner / city planner）
